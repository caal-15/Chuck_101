...
public class RhodeyPlayer extends Player
{
   ...
   fun int shredChord(int chord[], int times, dur beat)
   {
       if(chord.cap() > keys.cap()) return 0;
       for(0 => int i; i < chord.cap(); i++) Std.mtof(chord[i]) => keys[i].freq;
       for(0 => int i; i < times; i++)
       {
           for(0 => int j; j < chord.cap(); j++) 1 => keys[j].noteOn;
           beat => now;    
       }
       return 1;
   }
  
   fun int shredChord(int chord[], float low, float high, int times, dur beat)
   {
       if(chord.cap() > keys.cap()) return 0;
       for(0 => int i; i < chord.cap(); i++) Std.mtof(chord[i]) => keys[i].freq;
       for(0 => int i; i < times; i++)
       {
           for(0 => int j; j < chord.cap(); j++) Math.random2f(0.8, 1.0) => keys[j].noteOn;
           beat => now;    
       }
       return 1;
   } 
